...
const updateData = () => {
    getWeatherData().then(data => {
        ...
        const indoorTempBox = indoor.getElementsByClassName('tempbox')[0];
        const outdoorTempBox = outdoor.getElementsByClassName('tempbox')[0];
        const indoorAQIBox = indoor.getElementsByClassName('aqibox')[0];
        const outdoorAQIBox = outdoor.getElementsByClassName('aqibox')[0];
        ...
        const indoorAQILabel = indoor.getElementsByClassName('aqilabel')[0];
        const outdoorAQILabel = outdoor.getElementsByClassName('aqilabel')[0];
        
        let bgColor = getTempColor(data.indoor.temp);
        let color = contrast(bgColor);
        indoorTempBox.style.backgroundColor = bgColor;
        indoorTempBox.style.color = color;

        bgColor = getTempColor(data.outdoor.temp);
        color = contrast(bgColor);

        outdoorTempBox.style.backgroundColor = bgColor;
        outdoorTempBox.style.color = color;

        indoorTemp.innerHTML = `<span class="display-2 _temp">${Math.round(data.indoor.temp * 10) / 10}</span><span class="h2">°F</span>`;
        outdoorTemp.innerHTML = `<span class="display-2 _temp">${Math.round(data.outdoor.temp * 10) / 10}</span><span class="h2">°F</span>`;
        ...
        indoorAQI.innerHTML = `${Math.round(data.indoor.aqi.aqi)}`;
        indoorAQI.style.color = data.indoor.aqi.fontColor;
        indoorAQIBox.style.backgroundColor = data.indoor.aqi.color;
        outdoorAQI.innerHTML = `${Math.round(data.outdoor.aqi.aqi)}`;
        outdoorAQI.style.color = data.outdoor.aqi.fontColor;
        outdoorAQIBox.style.backgroundColor = data.outdoor.aqi.color;

        indoorAQILabel.style.color = data.indoor.aqi.fontColor;
        outdoorAQILabel.style.color = data.outdoor.aqi.fontColor;
    });
};